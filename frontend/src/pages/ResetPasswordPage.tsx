...
export function ResetPasswordPage({ token }: { token: string }) {
  ...
  const [result, setResult] = useState<{ kind: "error" | "notice"; text: string } | null>(null);
  ...
  async function submit(e: FormEvent) {
    ...
    if (password.length < 8) {
      setResult({ kind: "error", text: "Password must be at least 8 characters" });
      return;
    }
    if (password !== confirm) {
    ...
    try {
      ...
      setResult(
        res.error
          ? { kind: "error", text: errorMessage(res.error, "Could not reset the password") }
